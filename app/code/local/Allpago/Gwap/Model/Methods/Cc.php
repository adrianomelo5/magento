 ...
 */
class Allpago_Gwap_Model_Methods_Cc extends Mage_Payment_Model_Method_Abstract {
    ...
    public function assignData($data) {
        ...
        //Mage::log($info->getCcNumber().' - '.$info->getCcType().' - '.$info->getCcType().' - '.$info->getCcExpYear().' - '.$info->getCcExpMonth(),null,'assignData.log');
    ...
    public function initialize($paymentAction, $stateObject)
    {
        ...
        $mGwap->setType(Mage::getStoreConfig('payment/gwap_cc/mc_type'));
        ...
        //Mage::log('initialize fim: '.print_r($this->_gwap->getData(),true),null,'fluxo.log');
        if (Mage::getStoreConfig('payment/gwap_cc/tipo_autorizacao')) {
    ...
    public function authorize(Varien_Object $payment, $amount) {
        ...
        //if($config->getDebug()){
            Mage::log(print_r($parameters,true),null,'cc_allpago.log');        
        //}        
    ...
    public function authorizeNow($order, $cc) {
        ...
        try {
            ...
            $gwap = $this->_gwap;
            //Se anti-fraude estiver ligado Status = authorized
            $gwap->setStatus(($this->getAntifraude($order) ? Allpago_Mc_Model_Mc::STATUS_AUTHORIZED : Allpago_Mc_Model_Mc::STATUS_CAPTUREPAYMENT));
            $gwap->setErrorCode(null);
            $gwap->setErrorMessage(null);
    ...
    public function cancelOrder($order, $errorMsg) {
        ...
        if ($qtd_tentativas >= $this->qtd_tentativas) {
            $log->add($gatewayPayment->getOrderId(), '+ Conversao', 'authorize()', 'error', 'Ocorreu um erro na autorização instantânea', $errorMsg);
    ...
    public function removeCredit($order){
        ...
        if($gwap->getStatus() == 'captured' 
            ...
            Mage::throwException('Não é possível estornar um pedido ainda não capturado.');
        }
    }    
    ...
    public function validate() {
        ...
        //Mage::log($ccNumber.' - '.$info->getCcType().' - '.$info->getCcType().' - '.$info->getCcExpYear().' - '.$info->getCcExpMonth(),null,'validationCC.log');
