...
class Allpago_Fcontrol_Model_Standard extends Mage_Core_Model_Abstract {
    ...
    public function enfileirar() {
        ...
        //Criar analise para pedidos autorizados
        $this->verificarAutorizados();
        
        //Atualizar status para pedidos capturados
        $this->verificarCapturados();
    ...
    private function verificarAutorizados(){
        ...
        foreach ($gatewayPayments as $gatewayPayment) {
            ...
            //setUpdateAt removido (bugfix addTimeFilter)
            //$gatewayPayment->setUpdatedAt(Mage::getModel('core/date')->date("Y-m-d H:i:s"));
            $gatewayPayment->save();
            //Define os dados
            $fcontrolOrder = Mage::getModel('fcontrol/orders');
    ...
    private function verificarCapturados(){
        
        //Carrega todos os pedidos capturados, pendentes de analise
        $gatewayPayments = Mage::getModel('allpago_mc/payment')->getCollection()->addStatusFilter('captured');
        $gatewayPayments->getSelect()
            ->joinInner(
            "allpago_fcontrol_orders AS fcontrol",
            "main_table.order_id = fcontrol.order_id AND fcontrol.status <> 'finished'");
        
        //Percorre todos os pedidos autorizados
        foreach ($gatewayPayments as $gatewayPayment) {
            //Muda o status da tabela do fcontrol
            $fcontrolOrder = Mage::getModel('fcontrol/orders');
            $fcontrolOrder->setOrderId($gatewayPayment->getOrderId());
            $fcontrolOrder->setStatus('finished');
            $fcontrolOrder->setUpdatedAt(Mage::getModel('core/date')->date("Y-m-d H:i:s"));
            $fcontrolOrder->save();
        }
    }    
