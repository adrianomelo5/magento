 ...
 */
class Allpago_Gwap_Model_Methods_2cc extends Mage_Payment_Model_Method_Cc {
    ...
    public function assignData($data) {
        ...
        $info->setCcType($data->getData('gwap_2cc_type'))
                ...
                ->setCcLast4(substr(preg_replace("/[^a-zA-Z0-9\s]/", "", $data->getData('gwap_2cc_number')), -4))
                ...
                ->setAdditionalInformation('gwapCcLast4_2', substr(preg_replace("/[^a-zA-Z0-9\s]/", "", $data->getData('gwap_2cc_number2')),-4))
                ...
                Mage::getModel('core/session')->setGwapCcId(preg_replace("/[^a-zA-Z0-9\s]/", "", $info->getCcCid()));
                Mage::getModel('core/session')->setGwapCcCcNumber(preg_replace("/[^a-zA-Z0-9\s]/", "", $info->getCcNumber()));
                
        //if(Mage::getStoreConfig('payment/gwap_oneclick/active')){
        //     $info->setAdditionalInformation('GwapCheckOneclick', $data->getGwapCcCheckOneclick());
        //}        
    ...
    public function authorize(Varien_Object $payment, $amount) {
        ...
        $captureParams1 = $this->prepareCaptureRequestParameters($response1,$cc);
        $captureParams2 = $this->prepareCaptureRequestParameters($response2,$cc);
    ...
    public function authorizeNow($order, $cc) {
        ...
        try {
            ...
            $gatewayPayment->setStatus((Mage::helper('gwap')->getAntifraude($order) ? Allpago_Mc_Model_Mc::STATUS_AUTHORIZED : Allpago_Mc_Model_Mc::STATUS_CAPTUREPAYMENT));
    ...
    private function prepareCommonParameters($ccType) {
        ...
        $parameters['TRANSACTION.CHANNEL'] = $this->getTransactionChannel($config,$ccType);
    ...
    private function prepareCaptureRequestParameters($authorizationResponse,$data) {
        ...
        $parameters = array();
        $parameters = $this->prepareCommonParameters($data->getCcType());
    ...
    private function getTransactionChannel($config,$ccType) {
    ...
    public function _validateSecondCc() {
        ...
        if (in_array($info->getAdditionalInformation('gwapCcType2').'_C', $availableTypesC)
            ...
            if ($this->validateCcNum($ccNumber2)
                    // Other credit card type number validation
                    || ($helper->OtherCcType($info->getAdditionalInformation('gwapCcType2')) && $this->validateCcNumOther($ccNumber2))) {
        ...
        if ($ccType != 'SS' && !$this->_validateExpDate($info->getAdditionalInformation('gwapCcExpYear2'), $info->getAdditionalInformation('gwapCcExpMonth2'))) {
